...
const PROBLEM_CATEGORIES = [
  { id: "teror", emoji: "📵", title: "Diteror penagih (DC pinjol)", desc: "Ditelepon terus, diancam, data disebar? Ketahui batas yang boleh dan tidak.", match: ["teror", "dc", "sebar", "ancam"] },
  { id: "utang", emoji: "💳", title: "Terlilit utang & blacklist", desc: "Galbay, masuk SLIK, skor kredit jelek — apa akibatnya dan jalan keluarnya.", match: ["galbay", "blacklist", "slik", "skor", "tidak-dibayar", "akibat"] },
  { id: "lapor", emoji: "📣", title: "Mau melapor pinjol ilegal", desc: "Ke mana, lewat kanal apa, dan berkas apa yang perlu disiapkan.", match: ["lapor", "melaporkan", "ilegal"] },
  { id: "bunga", emoji: "🧮", title: "Cek bunga & legalitas", desc: "Bunga maksimal menurut OJK dan cara memastikan aplikasi terdaftar.", match: ["bunga", "maksimal", "legal", "cek"] },
];

const TOOLS = [
  { href: "/pinjol/cek-legalitas/", emoji: "🔎", title: "Cek Legalitas Pinjol", desc: "Terdaftar OJK atau tidak?" },
  { href: "/pinjol/cek-ilegal/", emoji: "🚩", title: "Pinjol Ini Ilegal?", desc: "Checklist tanda bahaya." },
  { href: "/kalkulator/bunga-harian/", emoji: "🧮", title: "Kalkulator Bunga Maksimal", desc: "Batas bunga & denda OJK 2026." },
  { href: "/kalkulator/simulasi-restrukturisasi/", emoji: "📉", title: "Simulasi Restrukturisasi", desc: "Bandingkan cicilan." },
];

function categorize(slug: string, keyword?: string) {
  const hay = `${slug} ${keyword ?? ""}`.toLowerCase();
  for (const cat of PROBLEM_CATEGORIES) if (cat.match.some((m) => hay.includes(m))) return cat.id;
  return null;
}
    ...
    <div>
      <section className="bg-gradient-to-b from-brand-50 to-paper border-b border-brand-100">
        <div className="mx-auto max-w-3xl px-4 py-14 sm:py-20 text-center">
          <p className="text-sm font-semibold text-brand-600 uppercase tracking-wide mb-3">{siteConfig.slogan}</p>
          ...
          <a href="#masalah" className="mt-8 inline-flex items-center rounded-lg bg-brand-500 px-6 py-3 text-white font-medium hover:bg-brand-600">
      ...
      <section id="masalah" className="mx-auto max-w-4xl px-4 py-8 scroll-mt-20">
        ...
        <div className="grid gap-8">
          {PROBLEM_CATEGORIES.map((cat) => {
            const items = articles.filter((a) => categorize(a.slug, a.frontmatter.target_keyword) === cat.id);
            return (
              <div key={cat.id} className="rounded-2xl border border-brand-100 bg-white p-5">
                ...
                {items.length > 0 ? (
                  <ul className="mt-4 divide-y divide-brand-50">
                    {items.map((a) => (
                      <li key={a.slug}>
                        <Link href={`/pinjol/${a.slug}/`} className="flex items-center justify-between gap-3 py-2.5 text-ink hover:text-brand-600 group">
                  ...
                  <p className="mt-4 text-sm text-ink-muted italic">Panduan untuk kategori ini sedang disiapkan.</p>
                )}
              </div>
            );
          })}
      ...
      <section id="alat" className="mx-auto max-w-4xl px-4 py-8 scroll-mt-20">
        <h2 className="text-2xl font-bold text-ink mb-1">Alat bantu</h2>
        <p className="text-ink-soft mb-6">Periksa sendiri dalam hitungan detik.</p>
        <div className="grid gap-4 sm:grid-cols-2">
          {TOOLS.map((t) => (
            <Link key={t.href} href={t.href} className="rounded-xl border border-brand-100 bg-white p-4 flex items-start gap-3 hover:border-brand-300 hover:shadow-sm transition">
