...
Signal:
  ?윟 green  ??layer 1&2 bersih -> published: true (boleh auto-publish)
  ?윞 yellow ??ada WARN / kutipan belum terverifikasi -> published: false (antre tinjauan)
...
def to_mdx(slug, keyword, body, verified_results, queue, warns):
    ...
    fm = {
        ...
        "changelog": [{"date": str(date.today()),
                       "note": f"Draft otomatis ??gate 1쨌2 ({queue})"}],
